# -*- coding: utf-8 -*-
"""S11a: L3 チャネル (b_δ, b_Δ) 単独のループゲイン測定 (指示書 §4/§5)。

同一シード・同一 L2 経路の on/off 対で g = 1 − √(Var_off/Var_on) を推定。
日次 (指示書の式) と 30 分帯域 (ループが実際に増幅する側) の両方を出す。
...
def main() -> int:
    ...
    for b_d, b_p, b_n in GRID:
        ...
        print(
            ...
            f"u_sd={med('u_sd'):.2f} nt_max={med('nt_max'):.3f} "
            f"sp_med={med('spread_median') * 100:.1f}t "
