...
役割
----
log σ の緩慢帯域に加算する決定論的成分を作る。目的は統計的リアリズムの向上では
なく、ボラティリティ・レジームの決定論的な再現性と制御可能性 — 同一のカオス
初期値なら、確率的な部分 (シード) が違っても同じレジーム構造が現れる。これが
複数シナリオ比較・戦略頑健性テストでの価値になる (S5 設計要件)。

再現性の要件 をこのモジュールが一手に引き受ける:
...
- ディスクキャッシュ: 長期 run では丸め誤差の蓄積で環境が違うと軌道が分岐し
  ...
  再生成する (キャッシュはあくまで高速化と再現性の証拠であって権威ではない —
...
滑らかな不規則振動で周辺分布が単峰 — 緩慢成分の滑らかな搬送波という χ₂ の
役割に合う (推奨)。

遅延微分方程式の RK4
--------------------
RK4 の各ステージは遅延値 x(t-tau), x(t+h/2-tau), x(t+h) を要求する。
``tau/h`` を整数に強制することで t-tau と t+h-tau は履歴グリッド上の点になり、
半ステージ点は隣接 2 点の線形補間 (= 平均) で与える。これは高次精度ではないが、
カオス軌道に正確な積分は存在しない (指数感度で必ず発散する) — 要件は
完全に固定された決定論的スキームであることと、アトラクタの性質 (正の
...
def mackey_glass(
    ...
    初期条件はt <= 0 で恒等的に ``ic``の定数履歴。遅延方程式の初期値は
    関数 (遅延区間の履歴全体) なので、スカラー 1 つで完全に指定できる定数履歴を
    採用する (設計要件: config に明示できる形)。
    """
...
def chaos_generate(
    ...
    再計算して照合する。照合はロードのたびに行う — キャッシュを信じるのでは
    なく使ってよいことを毎回証明する(壊れた/別環境のキャッシュを黙って使うと
    再現性の主張全体が崩れるため)。
    """
...
def chi_window(
    ...
    同一の MG 系を異なる初期値・異なる時間写像で回す (§3.1 の推奨構成)。
