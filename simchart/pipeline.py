...
駆動方式について
----------------
S0 はL0 が張った時間グリッド上で L2 を一括生成し、L3 がそれをそのまま観測する
というグリッド駆動である。しかし S6 で L3 がイベント駆動になると、主役は
...
#: 各段階の不変性照合の基準となる直前段階。
#: S2 の合否はS1 から何が変わらなかったかで決まる (S2 設計要件)。
...
class GridDriver:
    ...
    アーキテクチャ —「L2 は全期間を先に生成し、L3 のループでは補間参照
...
class AssetPayload:
    ...
    #: 日中のみの連続経路なので、クローズ・トゥ・クローズ系列を作るには
...
def run_multi(config: Config) -> MultiAssetResult:
    ...
    1. 共通因子状態を ``cross.*`` ストリームから 1 回だけ生成する
    ...
    3. 資産 0 (参照資産) はフル結果を保持、他はクロス測定素材だけ残す。
       メモリピークを 1 資産分に抑えるため資産 0 を最後に回す
    """
...
def factor_degeneracy_check(config: Config, n_days: int = 30) -> dict[str, Any]:
    """§8.3 の実体: 因子コードパスを通しても退化条件で S12 と一致するか。
...
def determinism_check(config: Config, first: StageResult | None = None) -> dict[str, Any]:
    ...
    ハッシュ一致は同じバイト列の十分条件としては強いが、どの配列が壊れたかを
    切り分けられないため、両方記録する。
    """
...
def rng_stability_check(config: Config, n_draws: int | None = None) -> dict[str, Any]:
    ...
    新機能の効果なのか乱数がずれただけなのか区別できなくなる。これは
...
def rng_diffusion_check(config: Config, result: StageResult) -> dict[str, Any]:
    ...
    同じ個数を引いた列のダイジェストを期待値として、パイプラインが実際に
...
def scale_invariance_check(config: Config, reference_result: StageResult) -> dict[str, Any]:
    ...
    一致することを確認する。1 ステップあたり切替確率型の実装はここで落ちる。
    ...
    # 落ちうる。その場合は板なし対照に切り替えて潜在側のみで判定する —
    ...
    # 脱相関させ、2 解像度は独立な板実現になる (実測: gph_d 差 0.11)。
...
def baseline_invariance_check(
    ...
    (S2 設計要件)。同一シードなら S1 のストリーム (拡散・MSM・OU) は名前ハッシュ
    RNG によりビット単位で不変のはずで、日次統計の差はラフ成分の追加効果だけになる。

    - gph_d (±0.03) が最重要 — 動いたらスケール分離の失敗であり、ラフ成分が
      MSM/OU の帯域 (1〜100 日) に漏れている (診断手順は設計要件)
    - RNG の証人: MSM の成分別切替回数・占有率、OU の x0・経路統計が JSON の
      float 往復 (repr 17 桁) で厳密一致すること — S1 のストリームに 1 draw
      でも触れていれば一致しない
    """
    from .report import load_metrics
    ...
    # S6 (κ=0 の板): 観測系列が L2 の p* から ZI 板のミッドに入れ替わる。
    ...
    # S10 (κ>0): 観測レジームがもう一度変わる (切断ミッド → 結合ミッド)。
    # S9 基準との観測照合は「変わらないこと」ではなく変わることが正解なので
    ...
    # 両段階に潜在 log sigma の GPH ((3) の構造の直接測定) があればそちらで判定する。
    # 観測 |r| の GPH は S3 のジャンプ・レバレッジが加える白色成分で下方バイアス
    # されるため、S3 以降は潜在側が本判定で観測側は記録になる。
    #
    # S5 (chi_2 有効) では判定の帯域を 0.65 → 0.50 に移す (2026-08-21 裁定)。
    # 設計要件はピークを 20〜40 日に置けとgph_d ±0.03を同時に要求するが、
    # 帯域 0.65 の測定帯は周期 >= 20 日で設計した 30 日線を必ず含む — 実測で
    ...
    if same_horizon and base_lat is not None and cur_lat is not None:
        # 同一視野なら潜在 d の直接比較が最強 (L2 凍結段階ではビット一致
    ...
    # 乗って γ が設計の帰結として動く (実測 0.399 → 0.475、+19%)。
    # 形状の質はゲート対象の R² 非劣化が引き続き守る (実測 0.865 → 0.858 合格)。
    ...
    #
    # S4 (ON 有効時) では判定しない — この標本量では検定として成立しないから。
    ...
    # 理論上は ON が総分散の share を取ると日中系列の超過尖度が 1/(1-share)
    ...
    # 通っている。分散配分そのものは日次 RV の中央値比 0.8014 (設計 0.80) で
    ...
    # H_latent の不変 (S3 設計要件: レバレッジ相関は粗さを変えない)。
    ...
    # 視野 (n_days) が基準と違うと、切替回数・経路統計・lam_eff は当然一致
    # しない (S6 は 500 日、S5 基準は 5000 日 — 検証規模)。その場合の
    ...
    # S10 (κ>0, c_vol>0) でも維持 — 板は p*/log σ を読むが書かない。
