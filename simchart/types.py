...
配列ではなく補間可能なオブジェクトとして定義してある。理由は S10 で L3 が
...
class PriceProcess:
    ...
    L2 のグリッドを細かくするが第一で、どうしてもブラウン橋が要るなら
    ...
    def _interp(self, q: np.ndarray, values: np.ndarray) -> np.ndarray:
        """線形補間。グリッド点上では丸め誤差なしに格納値そのものを返す。
...
class Observation:
    ...
    def to_bars(self, bar_seconds: float) -> "BarSeries":
        ...
        その時刻以前の最後の観測値を取る。セッション長が ``bar_seconds`` で
        割り切れない場合、末尾の端数は捨てる。
