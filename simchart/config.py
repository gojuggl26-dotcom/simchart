...
#: 全段階分を先に宣言する方針 (S0 §4) の perp 版 — L4 まで全部宣言し、
...
#: (テストが全 bool フラグはどちらかの台帳に載っていることを強制し、
...
class ValidationConfig:
    ...
    ここに入るのは測定器の設定であってモデルの設定ではない。段階をまたいで
    ...
    # --- S2 追加: 粗さ指数 H の測定器設定 ---
    # H の測定窓 (5 分〜4 時間) と GPH の測定窓 (1〜100 日) を重ねないこと。
    # 重ねると互いに汚染し、どちらの推定も信用できなくなる (S2 設計要件)。
    ...
    # S2 の合否は何が増えたかではなく何が変わらなかったかで決まる。
    inv_tol_gph_d_abs: float = 0.03  # 最重要。動いたらスケール分離失敗 
...
class Config:
    ...
    # 非定常な fBm/Volterra ではなく定常な fOU を使う (非定常だと分散の増大が
    ...
    #: Hurst 指数 (設計要件の許容範囲 0.08〜0.15)。既定 0.08 の理由: H_latent ゲートは
    ...
    # サイズはべき則ではなく指数 (Kou): べき則は集計してもテール指数が変わらず
    # 集計正規性 ((18)) を阻害する。指数なら集計で相対テールが薄れ、Hill α が
    # スケールとともに上昇する。α ≈ 3〜5 は有限標本の性質として狙う (§3.2)。
    # λ_jump_per_year の設計要件目安 (20〜60/年) は η_d 15〜25 と JV share 5〜15%
    ...
    #: 中速レバレッジ成分 — 既定は無効 (var=0) (2026-08-20 設計判断)。
    #: 経緯: per-step 相関では corr(r_t, RV_{t+1}) の理論上限が ~-0.06 で設計要件の
    ...
    #: 記憶が削れて gph_d が S2 から最大 -0.15 動く** ((3) の破壊)。(3) の保全を優先し
    #: 中速は無効化、lev ゲートは実測整合帯 [-0.08, -0.005] に変更。レバレッジ
    #: 水準の残りは S10 の板側チャンネルに委ねる (§5.3 の弱い側を狙う)。
    ...
    # --- L0 / S4: 日内季節性 (phi) ---
    # 対象市場は米国株相当の連続単一セッション (6.5 時間) — 2026-08-20 指定。
    ...
    #: 係数はphi^2 の起伏比 = 4.5 ( 3〜6 の中央)を満たすよう
    ...
    #: S4 では定義のみで消費されない (L1 がスタブのため)。S7 で使う。
    ...
    # ほとんど進まないので、別レジームとして c_ON を直接指定する 。
    #: クローズ・トゥ・クローズ分散に占める ON ギャップの寄与 (設計値)。
    #: 手元に実データが無いため文献値 (米国株 15〜25%) の中央を設定値として使い、
    #: ゲートは実測がこの設定値と一致するかを見る (実装の検定であって
    #: 市場の真値の主張ではない — 2026-08-20 設計判断)。
    ...
    # 目的は統計的リアリズムではなくボラ・レジームの決定論的な再現性と制御 —
    ...
    chaos_dt: float = 0.1
    #: 初期条件 = t<=0 の定数履歴。遅延方程式の初期値は関数 (履歴全体) なので、
    ...
    #: (同一シードで chi on/off の gph_d 差) で確定する — 訂正どおり
    ...
    # 6 次元 {成行,指値,取消}×{買,売} に符号対称制約 Φ[買X→買Y]=Φ[買X→売Y] を
    # 課す 。(11) 符号 ACF は S8 のメタオーダー分割の担当で、Hawkes の
    # 交差励起に符号相関を持たせると二重計上になり γ・β の整合が崩れる。この制約の
    # もとで 6D は型レベル 3D Hawkes + iid 符号と厳密に等価 (6×6 ブロック行列
    ...
    #: 構造は目安 (対角支配 + 成行→取消 = 約定を見て指値を引く挙動 =
    #: (13) の源)。値は scripts/calibrate_s7_hawkes.py の出力 — 定常レートを **S6 の
    ...
    #: 指数和カーネルの時定数 [秒]。最遅 300 秒 (上限 1 時間の内側) —
    #: 日次に伸ばすと S10 で MSM の帯域と競合して (3) を壊す。Hawkes は秒〜分の
    ...
    #: 設計要件の予算表のW_t (S7, 25%)は本実装に存在しない — S7 の
    ...
    #: (帯 10〜30)。周期は非共鳴に離調する: 当初の 15 日は χ₂ (30 日) と
    ...
    # κ=0 で L2 と完全に切り離す。この段階の価格には (1)(3)(4)(8)(16)(18) は現れない —
    # それが正しい状態 。L2 の性質が観測に現れるのは S10 の結合から。
    enable_book: bool = False  # S6
    #: ティックサイズ (価格単位)。p0=100 で 0.01 = 1bp。small tick レジーム
    #: (スプレッド 2〜5 ティック、板構造が観測可能)。large tick は板の別世界
    #: (キュー長が全て) なので S9 で別 config として分岐する 。
    ...
    #: 板上の各注文が独立に取り消される率 [1/日]。
    book_delta_cancel: float = 5.0
    #: 配置べき則 P(Δ) ∝ (Δ+Δ0)^-(1+μ) の指数 (帯 [0.6, 1.5])。
    ...
    #: サイズ分布: w·ラウンドロット + (1-w)·Pareto 。単位はロット。
    ...
    #: イベントごとに記録する best±N ティックのデプス 。
    book_depth_ticks: int = 10
    #: ウォームアップ (統計収集から除外する日数、設計要件)。
    ...
    # --- L3 / S8: メタオーダー分割 ((11) 約定符号の長期記憶) ---
    # Hawkes がいつを決め、メタオーダーはどちらの符号かだけを決める
    # (役割分離 — タイミング統計と分岐比は S7 から不変であること)。
    enable_metaorder: bool = False  # S8
    #: Pareto 長さの裾指数。定義域は開区間 (1, 2) — α≤1 は E[N] 発散で
    #: プールが定常にならず、α≥2 は Var(N) 有限で長期記憶が消える 。
    ...
    #: ψ は C(1) の水準を動かし、減衰指数 γ は動かさない (§4.4 — 別々に同定)。
    meta_psi: float = 0.6
    #: Poisson 到着の供給率 ρ = λ_meta·E[N] / (ψ·λ_MO)。
    #: 厳密に 1 にしてはならない: 子需要と供給が釣り合う臨界負荷では、E[N²]=∞
    #: (α<2) のプール占有が帰無再帰的にさまよい定常ゲートが成立しない
    ...
    #: False で時間優先を保持する取引所の変種 。
    book_iceberg_refill_tail: bool = True

    # --- L3 / S9: queue-reactive (状態依存の意思決定層) ---
    # 強度は状態依存にしない (採用案)。Hawkes が「いつ・種別・
    # サイド」を決め、S9 は種別を所与としてどこに置くか・どれを取り消すか
    # だけを板の状態から決める — 分岐比 n̂ は構造的に S7 のまま。
    # §3.3 の平均 1 乗法変調 g_m は使っていない (意思決定層で足りることを
    ...
    #: ref はスプレッド中央値 (2) ではなく 1 に置く: 中央値を基準にすると
    ...
    #: h(Δ, s) の形状 (§5): 板内配置の距離分布をべき則 (0)から「d=1 の重みで
    ...
    #: 既定は中立 (decay=0, floor=1, len=0, back=0 — 一様選択と選択・消費
    #: ともに厳密同値)。理由 (250 日 × 6 シード実測): 前方傾斜の取消は small tick
    #: レジームでは板の前面を薄くして赤字指標を全て悪化させる (ノイズ約定の
    #: 戻り率 0.29→0.12、β −0.25→−0.27)。ハンプ ((20)) は前方消耗 + 板内配置だけで
    ...
    #: γ ±0.05 のアブレーションが必須 ((11) の二重計上リスク)。
    qr_obi_bias: float = 0.0

    #: S9 fallback (§8.2): 明示的な uncertainty zones 層。板は既にティック
    #: 格子上で動くので常用しない — queue-reactive の較正で η が範囲に
    ...
    #: S10c: L2 の緩慢ボラ → L1 活動度のリンク :
    #: Z_t = exp(c_vol·V_t − c_vol²/2), V_t = (MA_w(log σ − log φ_σ) − m_V)/s_V
    #: Z は Hawkes のベースラインのみに乗る (カーネル不変 → n 保存、φ と同じ
    ...
    #: V の標準化スケール s_V。較正実測 (MA_3d 系列の定常 SD — 価格層のみ
    ...
    #: 信号は水準ではなく驚き : u_t = log(RV_short/RV_long)。
    ...
    #: RV 水準を ×1.65 押し上げた (S11e 実測)。c を引いて平均中立にする —
    ...
    #: u の中心化定数。u の定義は字義 (両 RV の算術 EWMA の
    #: log 比) から 対数域デトレンド u = log RV_s − EWMA_long(log RV_s) に
    #: 変更した。字義の形は自らの要件定常で u≈0を二重に破る:
    ...
    #: n_t のレンジ 。ハード上限 0.97 — S12 の χ₃ 変調の余地
    #: 0.07 を残すため n_max は 0.90 を推奨値とする (0.97 に張り付けない)。
    fb_n_min: float = 0.75
    fb_n_max: float = 0.90
    #: RV EWMA の半減期 (目安: 短 5〜30 分、長 1〜5 日)。
    ...
    n_assets: int = 1
    #: 各資産の共通因子ローディング β_i (設計要件: z_i = β_i z_F +
    ...
    #: 24/7・年 365 日)。分岐はレイヤー実装の差し替えで行い (l0_calendar の
    ...
    def _check_s1_params(self) -> None:
        ...
        if self.enable_rough:
            if not (0.0 < self.rough_hurst < 0.5):
                raise ValueError(
                    ...
                    " (H >= 0.5 はラフではなく持続的で、長スケールの記憶を汚染する)"
                )
        ...
        if self.enable_jump:
            if self.jump_eta_up <= 1.0:
                raise ValueError(
                    ...
                    f" マルチンゲール補償項 k が定義できません (S3 設計要件)。"
                )
        ...
        if self.enable_hawkes:
            ...
            if max(self.hawkes_tau_seconds) > 3600.0:
                raise ValueError(
                    ...
                    f" 日次帯域に食い込むと S10 で MSM と競合して (3) が壊れる 。"
                )
        ...
        if self.enable_feedback:
            ...
            if self.fb_n_max >= 0.97:
                raise ValueError(
                    f"fb_n_max = {self.fb_n_max} ≥ 0.97 (ハード上限、設計要件)。"
        ...
        if allocated > self.vol_var_budget_total + 1e-12:
            raise ValueError(
                ...
                f" ({self.vol_var_budget_total}) を超えています (設計要件:"
                f" chi_2 の 25% 超は (3)(18) を薄めるため禁止)。"
            )
        self._check_s13()
    ...
    def without_book(self) -> "Config":
        ...
        動いているとフラグ off + 非既定パラメータの暗黙 no-op ガードに当たる。
    ...
    def n1_config(self) -> "Config":
        ...
        因子パラメータを既定値に戻すと N=1 は S12 と同一コードパスになる
...
def _coerce(cls: type, data: Mapping[str, Any]) -> dict[str, Any]:
    ...
    入れ子の list (hawkes_a のような tuple[tuple, ...]) は再帰的に tuple 化
    する。外側だけ変換すると (list, list, ...) の tuple になり、既定値との比較や
    フラグ off + 非既定パラメータガードが誤発火する (to_dict 往復で実際に発火)。
    """
