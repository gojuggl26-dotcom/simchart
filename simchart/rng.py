...
崩れると段階間の差分が新機能の効果なのか乱数がずれただけなのか区別できず、
...
#: 設計要件 で宣言されたストリーム名。S0 では大半が未使用だが、名前は先に確定させる。
...
class RNGRegistry:
    ...
    strict:
        ...
        有効に見える別の系列を静かに返すという最悪の壊れ方をするため、
        既定で弾く。
    extra_streams:
    ...
    Notes
    -----
    同じ名前で 2 回 ``get`` すると同一の Generator オブジェクトが返る
...
def asset_stream_name(name: str, asset_index: int) -> str:
    ...
    n_assets=1 は S12 と厳密一致を成立させる: N=1 では因子パラメータが
...
class AssetStreamView:
    ...
    層のコードは 1 文字も変えずに多資産化できる。共通因子 (``cross.*``) は
