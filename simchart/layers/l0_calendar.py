...
- ``phi_sigma``: ``(1/T) ∫ phi^2 du = 1`` — 加算されるのは分散なので二乗の平均。
...
def fourier_profile(
    ...
    定数項を 1 に固定してあるので、係数は平均からの相対的な起伏を表す。
...
class ConstantCalendar:
    ...
    def intraday_position(self, t: np.ndarray) -> np.ndarray:
        ...
        セッションをまたいで連続にしない。後場の寄付は日の中盤ではなく
        セッションの開始なので、u は必ずセッション内で 0 から始める
        (分割セッションを入れるときにここが効く)。
        """
...
class PerpCalendar(ConstantCalendar):
    ...
    - オーバーナイト・ギャップは存在しない (config が併用を弾く)
    - 日境界は UTC 0 時の集計上の区切りにすぎず、価格グリッドは
