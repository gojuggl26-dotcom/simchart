...
def intraday_profile(
    ...
    method:
        ...
        既定を中央値にしてあるのは S3 のジャンプに対する頑健性のため。
    ...
    (季節性が乗法変調だから)。定数は正規化で落ちるので、φ の形だけが残る。
    """
...
def _design(u: np.ndarray, n_harmonics: int, include_slope: bool) -> np.ndarray:
    ...
    生成側 (:func:`~simchart.layers.l0_calendar.fourier_profile`) と同じ基底を
...
def estimate_phi(
    ...
    2. その 対数を Fourier 基底へ最小二乗回帰して平滑化する。
...
def true_phi_bars(
    ...
    """カレンダーの真の φ_σ をバー粒度へ集約する。

    バーの φ はバー中心での φではなく φ² のバー内平均の平方根である:

    .. math:: \\varphi_k = \\sqrt{B \\int_{k/B}^{(k+1)/B} \\varphi_\\sigma^2(u)\\,du}

    バーリターンの分散は瞬間分散のバー内積分であり、加算されるのは φ² だから。
    ...
    -------------------------------------
    連続時間の積分ではなく、生成側と同じ離散和を使う。L2 は Euler-Maruyama で
    各ステップの左端の σ を使うので、バー k の実際の分散は
...
def profile_flatness(r_2d: np.ndarray, method: str = "median_abs") -> dict[str, Any]:
    ...
    ----------------------------------------------
    平坦になったの基準は標本誤差なので、その見積りが甘いと判定が意味を失う。
    ...
    そこで分割標本で直接推定する: 日を前半・後半に分けて別々にプロファイルを
...
def acf_periodicity_test(
    ...
    細かいバーでの周期性なしは季節性が無い証拠にはならないので、主判定には
...
def spectral_periodicity_test(
    ...
    なぜ周波数領域なのか
    --------------------
    季節性は ``|r|`` に厳密に周期 B の決定論的成分を加える。周期成分の
    スペクトルは離散的で、力は基本周波数 ``2pi/B`` の整数倍 (高調波) だけに
    載る。一方ボラの長期記憶は連続スペクトルで、高調波の位置に特別なことは
    起きない。したがって周波数領域では両者が原理的に分離でき、ACF でやろうとした
    ときの周期成分と滑らかな減衰が混ざる問題 (:func:`acf_periodicity_test`) が
    そもそも生じない。
...
def overnight_stats(
    ...
    - 尖度 — ON は単一の情報ショックが溜まって一度に出るので日中日次より厚い
    ...
    if sigma_close is not None:
        ...
        if s.shape == g.shape and s.std() > 0:
            ...
            # この相関に> 0.5のような固定閾値を当ててはならない。
            # 構造上 sigma_ON = c_ON sigma_close なので潜在の相関は厳密に 1 だが、
            ...
            # 判定は0 より有意に大きいかで行い、この予測値は解釈の補助に使う。
...
def time_change_by_phi_lambda(
    ...
    ``(1/T)∫phi du = 1`` 正規化は変更後の時間の総量がセッション長と等しいことを
    意味するので、変更前後で単位が保たれる。
...
def coarsen(r_2d: np.ndarray, n_out: int) -> np.ndarray:
    ...
    リターンは対数価格の差なので、隣接するバーの和がそのまま粗いバーの
