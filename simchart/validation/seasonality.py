...
def acf_periodicity_test(
    ...
    **系統的に負へ偏る**。ボラの ACF はラグに対して減少**凸**なので、
    ...
    周期の段差が原理的に立たない。同一経路での実測は次のとおり:

    ====== ========== ============== =========
    バー   本/日      隣接 φ 差/平均  raw の z
    ====== ========== ============== =========
    1 分   390        0.008          検出不能
    15 分  26         0.111          +1.6
    30 分  13         0.215          +4.1
    ...
    したがってこの検定は **30 分足以上の粗いバー**で使うこと。細かいバーでの
    「周期性なし」は季節性が無い証拠にはならない。細かいバーでの主判定には
    :func:`profile_flatness` を使う (そちらは粒度に依らず検出力がある)。
