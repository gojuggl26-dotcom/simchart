...
役割の整理 (S5 指示書 §9)
--------------------------
これらの推定器は 3 つの対象に、**期待される結果が違う**まま適用される:
...
推定器についての注意
--------------------
- Rosenstein も GP も**決定論的データを前提**とする。確率過程に当てると Rosenstein
  は偽の正の傾きを出す (近傍が拡散で離れるため)。合成系列への適用値は
  「実データ解析でこう見える」という記録であって、カオスの証拠でも反証でもない
- MG(17) は**高コヒーレントな振動子** (実測: 周期 ~49.7 単位にパワーの 84%、
...
def test_0_1_chaos(
    ...
    - ``c`` は**決定論的な**固定格子 (乱数で引かない — このモジュールの系全体が
...
# pytest が名前で誤収集しないための印 (指示書 §10 の関数名を保つため改名しない)。
...
def bds_test(x: np.ndarray, max_dim: int = 4, max_points: int = 20000) -> dict:
    ...
    ★棄却 = 「カオス」ではない。BDS は iid からのあらゆる逸脱 (確率ボラ・
