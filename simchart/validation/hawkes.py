...
凹最大化になる — 大域最適が一意なので単一開始点の L-BFGS-B で足りる。

時間の規約: 立会時間を連結したセッション秒。エンジン自体が日境界を
...
分岐比の 3 経路 (設計要件の中心ゲート、Filimonov & Sornette 2015 の罠の実証):

- ``n_hat_raw`` — 季節性を除去せずに推定。日内 U 字によるイベント集中を
                       自己励起と誤認し、系統的に過大になるはず (> n + 0.03)
...
def estimate_phi_lambda(
    ...
    これは実データで可能な経路の再現なので、真の φ を一切参照しない。
...
def branching_three_ways(
    ...
    """raw / true-φ / est-φ̂ の 3 経路で n を再推定する (設計要件の中心ゲート)。
    ...
    それが実データで可能な経路の再現であり、est の帯 ±0.08 が緩い理由)。
    """
    times = np.asarray(times, dtype=np.float64)
    marks = np.asarray(marks, dtype=np.int64)
    if times.size < 1000:
...
def simulate_branching(
    ...
    thinning (エンジン) と数学的に等価な過程の別実装 — 実現レートと
