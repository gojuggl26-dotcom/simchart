...
尖度がスケールで変わらないことは S0 の中心的な主張である。実データでは
...
しかない。そこで 標本数がしきい値未満のスケールは記録のみ とし、ゲート
...
def zeta_q(
    ...
    ``zeta_q = q/2`` の直線、多重フラクタルなら上に凸の曲線になる。
...
def signature_plot(
    ...
    (有名な右下がりの曲線)。S0 にはノイズが無いので平坦になるのが正解であり、
...
def msm_diagnostics(msm_meta: dict | None, horizon_days: float | None = None) -> dict:
    ...
    これは切替動学の検証を担当する。合成式と正規化の検証はアンサンブル断面
...
def kurtosis_decay_fit(r_daily: np.ndarray, scales_days=(1, 2, 5, 10, 20, 50), min_obs: int = 100) -> dict:
    ...
    集計は重なり窓 (rolling sum) で行う。非重複ブロックだと粗いスケールの
...
def roughness_exponent(
    ...
    zeta_q が q に線形 (R^2) であることが単一フラクタルな粗さの確認になる。

    測定窓はラフ成分が支配する帯域 (既定 5 分〜4 時間) に限ること。GPH の
    測定窓 (1〜100 日) と重ねると互いに汚染し、どちらも信用できなくなる
    (S2 設計要件)。``scales_steps`` は入力系列のステップ数で与える。
    """
...
def daily_invariance_stats(result) -> dict:
    ...
    2 つの解像度の実行を同じ汎関数で測るための最小セット。Var(log sigma) は
...
def spectral_peak(
    ...
    日内季節性の帯域 (0.8〜1.2 日) に新規のピークを立てていないこと。
    後者は日周期帯のパワーシェアで測る — MG は 30〜40 日スケールの滑らかな
...
def cross_seed_correlation(paths: list[np.ndarray]) -> dict:
    ...
    呼び出し側は φ_σ (これも決定論成分) を除去してから渡すこと — 残すと
