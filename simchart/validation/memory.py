...
無いので実質的な違いは無いが、S4 でギャップを入れた瞬間に日をまたぐ差分が
...
def leverage_function(
    ...
    return ok(
        ...
        # 形状判定はL(1) 負 かつ 平均負で行う (2026-08-20 裁定)。
        shape_ok=bool(
...
def acf(x: np.ndarray, max_lag: int = 100, demean: bool = True) -> dict:
    ...
    ``x`` が 2 次元なら行をセッションとみなし、行内で完結するラグだけを使って
...
def ljung_box(x: np.ndarray, lags=(1, 5, 10, 20, 50), primary_lag: int | None = None) -> dict:
    ...
    出る (ラグ 5 本なら、独立系列でもどれかが 1% を切る確率は 1% より高い)。
...
def acf_powerlaw_fit(
    ...
    負に振れた点は捨てられる)、R^2 が曲線の直線性ではなくノイズ量を測る
