...
S12 (χ₁/χ₃ 注入) まで — L2 のボラ (χ₂)・L1 の活動リズム (χ₁)・分岐比の脆弱窓
(χ₃) が 3 本の独立カオス系列で決定論的に駆動され、「いつ市場が脆くなるか」が
シード非依存に再現される (n_t 窓相関 0.975)。個々の危機の発火は確率的 (窓カウント
ICC 0.145 = whale 供給の天井 — 設計どおり §8.1)。㉓ 危機 ~50/年・二相回復、
g ≈ 0.10、発散 0。約定時間 VR / √ 則 / レバレッジ水準 / 全体 Hill α 等は
「価格・流動性が取引によってしか動かない + 危機の存在は whale 供給」という構造の
未達として記録 (S10〜S12 節) — 気配再提示・流動性再参入チャネルが将来候補。**
...
| 予測 | 実測 | ゲート |
|---|---|---|
| ⑦ 希釈: 0.6354 × √(3/4) = 0.550 | **0.558** | [0.521, 0.585] ✓ 的中 |
