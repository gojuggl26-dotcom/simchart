...
S11 (RV フィードバック) まで — 実現ボラ → 板/注文流の負のループ (g ≈ 0.12、
発散 0) が配線され、㉓ 流動性危機が内生的に出る (49.9/年、無情報事象は 30 分で
0.35・1 日で全戻し、追いつき事象は情報的に継続)。⑧ は危機日除外バックボーン
(α 3.10) で判定 — 全体 α 2.68 は whale 頻度由来の既知の複合偏差。約定時間 VR /
√ 則 / レバレッジ水準等は「価格・流動性が取引によってしか動かない」構造の未達
として記録 (S10/S11 節) — 気配再提示・流動性再参入チャネルが S12+ の候補。**
...
★S10/S11 の帰結 (上のセクション): 壁時計側の赤字 (日次 VR・③・⑦・追随) は
閉じ、㉓ 内生的危機 (whale 供給 × フィードバック増幅、二相回復) も出た。
約定時間 VR / √ 則 / β̂ 理論関係 / ミッド signature / レバレッジ水準 /
全体 Hill α は**「価格・流動性が取引によってしか動かない」構造の未達**として
根拠つきで記録済み。埋める候補 = 気配再提示チャネル + 流動性提供者の再参入
(疲労予算) — S12 以降の設計判断に委ねる。

| 段階 | 内容 | 主な追加先 |
|---|---|---|
| S12 | カオス χ₁ / χ₃ | `layers/l1_activity.py`, ストリーム `l1.chaos`。★χ₃ の n_t 変調余地 0.07 は S11 が確保済み (n_max=0.90) |
