...
S0〜S13 の 13 段階で作る市場マイクロ構造シミュレータ。
今後、機能の追加やシミュレート手法の追加などを検討中である。

開発目的：ポートフォリオ最適化研究のため。実際の銘柄のマーケットデータの入手と保存にはコストがかかる。特にポートフォリオ最適化においては大量のマーケットデータが必要となるため、チャートシミュレータを作成し標本データをなるべく低コストで入手できるようにしたい。

仮定：このシミュレータは以下の仮定のもと、チャートをシミレートしている。

- **潜在価格が存在する**
- **市場現象を時間スケールごとに分離可能**
- **注文イベントは確率的に表現可能**
- **CLOBが価格形成の中心であること**
- **市場レジーム変化に決定論的カオス構造が存在すること**
...
- **S10** (`git tag S10-coupled`): κ 結合 — メタオーダー生成時の符号バイアス
  ...
  活動度 Z_t (⑦ 0.56)。L2 の性質が初めて観測に現れる。⑪ は残差符号テール窓で保存 (0.649)。
...
- **S12** (`git tag S12-chaos-l1`): χ₁/χ₃ 注入 — 同一 MG 系を非共鳴離調
  ...
  0.95)。窓は再現 (n_t 窓相関 0.975)・発火は確率的を二層計器化。近臨界レジーム (レート ×1.85 — 1/(1−n) の凸性) は設計帰結として
  計器を時変 n_t 対応に再整合
...
の実現が共通なので、これは別々の乱数で描いたそれらしいチャートの並置ではなく、
成分の効果そのものを示している 。

- **S0**: ボラが完全に平坦。ローソクの大きさが一様で、実チャートに見えない。
- **S1**: MSM の離散的な切替と緩慢 OU でボラが階段状に変動する。同じ乱数でも
  大きい足と小さい足の塊ができる
- **S2**: ラフ成分 (H≈0.1) が分単位の微細な価格変動を足す。日をまたぐ記憶は
  S1 から変えない
- **S3**: ボラ構造は S2 と同じまま、価格側にジャンプが入る。
