...
S7 (Hawkes 注文流) まで — L2 は構造凍結済み、観測価格は板のミッド、
注文流は自己励起でクラスタ化している。**
...
uv run python -m simchart.cli run --config configs/s7.yaml --seeds 10   # S7 本番 (約 100 秒)
...
(L1 は仕様保持者)。後続段階も「表の予定」より「層の物理」を優先すること。
