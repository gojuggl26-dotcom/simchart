...
S8 (メタオーダー分割) まで — L2 は構造凍結済み、観測価格は板のミッド、注文流は
自己励起でクラスタ化し、約定符号は冪則の長期記憶を持つ (板が未適応のため価格は
超拡散 — S9/S10 が埋める「インパクト赤字」として定量化済み)。**
...
uv run python -m simchart.cli run --config configs/s8.yaml --seeds 10   # S8 本番 (約 2 分)
...
(L1 は仕様保持者)。S8 のメタオーダーも同じく板カーネル内 (符号決定はイベント
ループの一部)。後続段階も「表の予定」より「層の物理」を優先すること。
