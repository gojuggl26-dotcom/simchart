...
**図の読み方** — S0-perp で株式版と違うのは「時計」と「セッションの有無」だけで、
価格モデルは同じ GBM である。

- **(1) 時計の違い**: equity は 1 日 = 6.5 時間 (23,400 秒) × 年 252 日で、
  セッションとセッションの間に**窓**が空く (S4 のオーバーナイト機構)。
...
- **(2) 同じ乱数・違う時計**: 同一シード・同一 σ̄ で時計だけを変えた対照。
  ...
  **2 本の経路は形がそっくりのまま振幅だけが √(252/365) = 0.831 倍**になる。
  ...
  これが「時間軸の単一情報源」(§4) が防いでいる事故そのもので、実際に
...
既知バイアス (−1/365) に対し **z = −0.33 = 系統バイアス無し**で、今回の 10 本が
...
`results/S0/charts` で、**株式のベースラインチャートを上書き**する。どちらも
