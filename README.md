...
S9 (queue-reactive) まで — L2 は構造凍結済み、観測価格は板のミッド、注文流は
自己励起 + 符号長期記憶、板は状態依存の意思決定層を持つ。インパクト赤字は
片側 (スプレッド緩和) が縮み、残り (変位のアンカー) は S10 の κ が埋める。**
...
★S10 が埋めるべき残存赤字 (S9 実測): VR_tt(1000) 2.92 → 0.9-1.1 /
β̂ −0.209 → +0.19±0.05 / サイズ傾き 0.92 → 0.4-0.7 / ミッド signature 比
0.65 → >1。機構は κ (メタオーダー符号の p\* 方向バイアス) — 変位のアンカー。
